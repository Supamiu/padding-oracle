# Soit C -> chiffré
# D -> Déchiffré
# P -> Plaintext
# i le bloc et j la position
# P28 = D28 ^ C18
# Le vecteur d'initialisation est 00000000b
import base64

import requests

C = bytearray.fromhex(
    "")

X = bytearray()
P = bytearray()
D = bytearray()


def main():
    current_block = bytearray.fromhex("0000000000000000")
    for i in range(0, 256):
        current_block[7] = i
        print("sending request #" + str(i))
        r = test(current_block, C[0:8])
        if r:
            print("found first one : " + print_block(current_block))
            break
    return 0


def print_block(block):
    return ''.join('{:02x}'.format(x) for x in block)


def test(my_bloc, bloc):
    data = concat_arrays(my_bloc, bloc)
    print_block(data)
    assert len(data) == 16
    res = requests.post("http://padding-oracle.cleverapps.io/",
                        base64.b64encode(data).decode('ascii'))
    return res.text[:1] == "1"


def concat_arrays(a, b):
    c = bytearray()
    c[:] = a
    for data in b:
        c.append(data)
    return c


if __name__ == "__main__":
    main()
